...
def main(args):
    ...
    # Build data loader
    train_data_loader, cnn_in_feature = get_loader(args.train_image_dir, args.train_sis_path, vocab, train_transform, args.batch_size, shuffle=True, num_workers=args.num_workers)
    val_data_loader, cnn_in_feature = get_loader(args.val_image_dir, args.val_sis_path, vocab, val_transform, args.batch_size, shuffle=False, num_workers=args.num_workers)

    encoder = EncoderStory(cnn_in_feature, args.img_feature_size, args.hidden_size, args.num_layers)
    ...
    for epoch in range(args.num_epochs):
        ...
        decoder.train()
        avg_loss = 0.0
        for bi, (image_stories, targets_set, lengths_set, photo_squence_set, album_ids_set, storyID) in enumerate(train_data_loader):
            decoder.zero_grad()
            encoder.zero_grad()
            loss = 0
            images = to_var(torch.stack(image_stories))

            features, _ = encoder(images, storyID)
