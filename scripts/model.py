...
class EncoderStory(nn.Module):
    def __init__(self, in_feature_size, img_feature_size, hidden_size, n_layers):
    ...
    def forward(self, story_images, storyId):
        imgPath = "/home/ashwinsr/resnetFeat152/" + str(storyId) + ".npz"
        if os.path.exists(imgPath):
            local_cnn = np.load(imgPath)
        else:
