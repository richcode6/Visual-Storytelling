...
def do_eval(feature, curr_key, curr_value, encoder, decoder, captions, vocab):
    encoder.eval()
    decoder.eval()
    sentences = []
    target_sentences = []
    print("For story :::: from train ")
    inference_results = decoder.inference(curr_key, curr_value, feature)
...
def main(args):
    ...
    Log_file = open("Perp_logger.txt", "w")
    ...
    encoder = EncoderStory(cnn_in_feature, args.img_feature_size, args.hidden_size, args.num_layers, args.proj_dim)
    decoder = DecoderStory(args.embed_size, args.hidden_size, vocab, args.hidden_size, args.proj_dim)
    print(encoder)
    print("--------------------------------------------------------")
    print(decoder)
    pretrained_epoch = 0
    ...
    for epoch in range(args.num_epochs):
        ...
        for bi, (image_stories, targets_set, lengths_set, photo_squence_set, album_ids_set, storyID, isPreLoaded) in enumerate(train_data_loader):
            ...
            # B x 5 x dim
            keys, values, features = encoder(images, storyID, isPreLoaded)

            for si, data in enumerate(zip(features, targets_set, lengths_set, keys, values)):
                feature = data[0]
                ...
                # 5 x L x vocab_dim
                outputs = decoder(curr_key, curr_value, feature, captions, lengths)

                for sj, result in enumerate(zip(outputs, captions, lengths)):
                    loss += criterion(result[0], result[1][0:result[2]].detach().cpu())
                if bi > 30 and si == 17:
                    print("For story :::: ", storyID[si], bi, si)
                    do_eval(feature, curr_key, curr_value, encoder, decoder, captions, vocab)
            avg_loss += loss.item()
        ...
        Log_file.write("Epoch " + str(epoch + 1) + " Average Train Loss: " + str(avg_loss) + " Average Train Perplexity: "+ str(np.exp(avg_loss)) + "\n")
        # Save the models
        ...
        Log_file.write(
            "Epoch VAL" + str(epoch + 1) + " Average val Loss: " + str(avg_loss) + " Average val Perplexity: " + str(np.exp(avg_loss)) + "\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default=' /project/ocean/home/ashwinsr/glacExp/imgAttModels/models/' ,
                        help='path for saving trained models')
    parser.add_argument('--image_size', type=int, default=224 ,
                        help='size for input images')
    parser.add_argument('--vocab_path', type=str, default='/project/ocean/home/ashwinsr/glacExp/imgAttModels/models/vocab.pkl',
    ...
    parser.add_argument('--proj_dim', type=int, default=512,
                        help='proj dim')
    parser.add_argument('--pretrained_epoch', type=int, default=11)
