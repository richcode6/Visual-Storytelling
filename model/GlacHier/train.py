...
def do_eval(feature, curr_key, curr_value, encoder, decoder, captions, vocab, text_key, text_val, desc_len):
    encoder.eval()
    decoder.eval()
    sentences = []
    target_sentences = []
    print("For story :::: from train ")
    inference_results = decoder.inference(curr_key, curr_value, feature, text_key, text_val, desc_len)
...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for bi, (image_stories, targets_set, lengths_set, photo_squence_set, album_ids_set, storyID, isPreLoaded, AllDescs, desc_length_set) in enumerate(train_data_loader):
            ...
            for si, data in enumerate(zip(features, targets_set, lengths_set, keys, values, text_key, text_val, desc_length_set)):
                feature = data[0]
                captions = to_var(data[1])
                lengths = data[2]
                curr_key = data[3]
                curr_value = data[4]
                text_key_curr = data[5]
                text_val_curr = data[6]
                desc_len_curr = data[7]
                #print("From Train: ", desc_len_curr)
                # 5 x L x vocab_dim
                #ind_lens = [data[i].shape for i in range(3,7)]
                #print(ind_lens)
                outputs = decoder(curr_key, curr_value, feature,
                                  captions, lengths, text_key_curr, text_val_curr, desc_len_curr)
                ...
                target_sentences = []
                if bi < 5 and si == 17:
                    print("TRAIN STORYID ", storyID[si])
                    do_eval(feature, curr_key, curr_value, encoder, decoder, captions,vocab, text_key_curr, text_val_curr, desc_len_curr)
        ...
        for bi, (image_stories, targets_set, lengths_set, photo_sequence_set, album_ids_set, storyID, isPreLoaded, AllDescs, desc_length_set) in enumerate(val_data_loader):
            ...
            for si, data in enumerate(zip(features, targets_set, lengths_set, keys, values, text_key, text_val, desc_length_set)):
                ...
                text_val_curr = data[6]
                desc_len_curr = data[7]
                #ind_lens = [data[i].shape for i in range(len(data))]
                #print(ind_lens)
                outputs = decoder(curr_key, curr_value, feature, captions, lengths, text_key_curr, text_val_curr, desc_len_curr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_epoch', type=int, default=15)
