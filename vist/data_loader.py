...
class VistDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        images = []
        Alldescs = []
        imgPath = "/project/ocean/home/rrnigam/vist/resnet_101_last3rd/" + str(story_id) + ".npz"
        ...
        if len(Alldescs) == 0:
            print(Alldescs)
            print("**** marked empty full set ---> ", story_id)
            exit(0)
...
def collate_fn(data):
    ...
    image_stories, caption_stories, photo_sequence_set, album_ids_set, storyID, preTrained, AllDescs = zip(*data)
    global_lengths = []
    targets_set = []
    lengths_set = []
    desc_set = []
    ...
    for desc in AllDescs:
        global_lengths.extend([len(imageDesc) for imageDesc in desc])

    for desc in AllDescs:
        lengths = [len(imageDesc) for imageDesc in desc]
        descriptions = np.zeros((len(desc), max(global_lengths))).tolist()
