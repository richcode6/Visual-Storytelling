import json
import pickle

class VIST:
    def __init__(self, ValidStories, sis_file = None, dii_file=None):
        if sis_file != None:
            sis_dataset = json.load(open(sis_file, 'r'))
            dii_dataset = json.load(open(dii_file, 'r'))
            #print("Loading: ")
            self.ValidStories = pickle.load(open(ValidStories, "rb"))
            print("Valid Stories: ", len(self.ValidStories), type(self.ValidStories))
            self.LoadAnnotations(sis_dataset)
            self.LoadDescriptions(dii_dataset)


    def LoadAnnotations(self, sis_dataset = None):
        images = {}
        stories = {}

        if 'images' in sis_dataset:
            for image in sis_dataset['images']:
                images[image['id']] = image

        if 'annotations' in sis_dataset:
            annotations = sis_dataset['annotations']
            for annotation in annotations:
                story_id = annotation[0]['story_id']
                if story_id in self.ValidStories:
                    stories[story_id] = stories.get(story_id, []) + [annotation[0]]

        self.images = images
        self.stories = stories


    def LoadDescriptions(self, dii_dataset = None):
        imagesDesc = {}
        if 'annotations' in dii_dataset:
            annotations = dii_dataset['annotations']
            for annotation in annotations:
                img_id = annotation[0]['photo_flickr_id']
                desc = annotation[0]['text']
                imagesDesc[img_id] = desc
                #print(img_id, desc)
                #exit(0)

        self.imagesDesc = imagesDesc
        #self.stories = stories

