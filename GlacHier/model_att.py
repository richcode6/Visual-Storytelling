...
class ImageAttention(nn.Module):
    ...
    def forward(self, decoder_hidden, encoder_key, encoder_value, state_len =None):
        ...
        #decoder_state = self.decoder_att(decoder_hidden)
...
class DecoderRNN(nn.Module):
    ...
    def forward_per_sentence(self, keys, values, features, captions, text_key, text_val, desc_len, length=0, tf=1.):
    ...
    def forward(self, keys, values, features, captions, lengths, text_key, text_val, desc_len):
        outputs = []
        for i, length in enumerate(lengths):
            # 1 x L x V
            output = self.forward_per_sentence(keys[i], values[i], features[i], captions[i],
                                               text_key[i], text_val[i], desc_len[i], lengths[i])
            outputs.append(output.squeeze(0))
         # 5 x L x V
        return outputs

    def inference(self, keys, values, features, text_key, text_val, desc_len):
        results = []
        ...
        for idx, feature in enumerate(features):
            ...
            self.init_encoder_state(keys[idx], values[idx], text_key[idx], text_val[idx], desc_len[idx])  # , text_key[idx], text_val[idx], desc_len[idx])
            # initialize state to last state of encoder, cell & context set to 0
            ...
            for i in range(50):
                #  L x V
                state, cell, _, context, outputs = self.f_next(state=state, cell=cell, context=context,
                                                               word_input=predicted)
                #print(outputs.size())
...
class EncoderStory(nn.Module):
    def __init__(self, in_feature_size, desc_vocab_size, embed_size, hidden_size, n_layers, proj_dim):
        ...
        self.img_dir = "/home/ashwinsr/resnetFeatModAlt/"
        ...
        self.text_key_project = MLP(hidden_size * 2, [], proj_dim)
        self.text_val_project = MLP(hidden_size* 2, [], proj_dim)
    ...
    def forward(self, story_images, storyId, prePicked, desc):
        ...
        rnn_out,_ = self.lstm(word_embed.view(-1, sizes[2], sizes[3]))
        #print(rnn_out.size())
        ...
        del word_embed
        return key, value, local_cnn, text_key, text_val, rnn_out
...
class DecoderStory(nn.Module):
    ...
    def forward(self, keys, values, story_feature, captions, lengths, text_key, text_val, desc_len):
        #print("From DecoderStory: ", len(desc_len))
        result = self.rnn(keys, values, story_feature, captions, lengths, text_key, text_val, desc_len)
        return result

    def inference(self, keys, values, story_feature, text_key, text_value, desc_len):
        result = self.rnn.inference(keys, values, story_feature, text_key, text_value, desc_len) #keys, values, features, text_key, text_val, desc_len
        return result
